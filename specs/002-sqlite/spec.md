...
- **FR-022**: SQLite deployments MUST attach the `sqitch.db` registry database to the same SQLite connection that executes user change scripts so that registry mutations and user migrations occur in a single atomic transaction. All deploy, verify, revert, and rework flows MUST `ATTACH` the registry database under the canonical alias used by Sqitch (`sqitch`), execute statements under a single transaction boundary, and guarantee that rollbacks unwind both user statements and registry state mutations together. Failing to attach, attaching to alternate aliases, or performing registry writes outside the shared transaction SHALL be treated as a parity-breaking defect.
