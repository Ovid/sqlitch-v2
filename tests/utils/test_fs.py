...
def test_resolve_plan_file_prefers_sqitch(tmp_path: Path) -> None:
...
def test_resolve_plan_file_supports_sqlitch_drop_in(tmp_path: Path) -> None:
...
def test_resolve_plan_file_conflict(tmp_path: Path) -> None:
    ...
    with pytest.raises(fs.ArtifactConflictError, match="sqitch.plan"):
        fs.resolve_plan_file(tmp_path)


def test_resolve_config_file_prefers_sqitch(tmp_path: Path) -> None:
...
def test_resolve_config_file_supports_sqlitch_drop_in(tmp_path: Path) -> None:
    config = tmp_path / "sqlitch.conf"
    config.write_text("[core]\nengine=pg\n", encoding="utf-8")
    ...
    assert result.path == config
    assert result.is_drop_in is True
    assert result.source_name == "sqlitch.conf"
...
def test_cleanup_artifacts_removes_files_and_directories(tmp_path: Path) -> None:
    file_path = tmp_path / "sqitch.plan"
    ...
    removed = fs.cleanup_artifacts(tmp_path, ["sqitch.plan", "deploy", "missing.file"])
