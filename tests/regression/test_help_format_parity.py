...
def test_gc_001_all_commands_support_help_flag():
    ...
    for cmd in COMMANDS:
        ...
        assert cmd in output_lower, \
            f"Help for '{cmd}' should mention command name"
        
        # Should have usage/synopsis or options section
        assert any(keyword in output_lower for keyword in ["usage", "options", "synopsis"]), \
...
def test_help_output_includes_description():
    ...
    for cmd in COMMANDS:
        ...
        assert len(result.output) > 50, \
            f"Help for '{cmd}' should provide substantial information"
...
def test_help_does_not_execute_command_logic():
    ...
    for cmd in ["add", "checkout", "rework"]:
        ...
        assert "missing" not in result.output.lower() or "usage" in result.output.lower(), \
