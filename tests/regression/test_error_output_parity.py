...
def test_gc_004_missing_args_error_to_stderr():
    ...
    for cmd in commands_with_required_args:
        ...
        assert result.exit_code == 2, f"Command '{cmd}' should fail with missing args"

        # Error message should exist
        assert len(result.output) > 0, f"Command '{cmd}' should have error output for missing args"
