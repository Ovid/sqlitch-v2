...
def test_gc_005_all_commands_reject_unknown_options():
    ...
    for cmd in COMMANDS:
        ...
        assert (
            "no such option" in output_lower
            or "unrecognized" in output_lower
...
def test_gc_005_unknown_option_prevents_execution():
    ...
    for args in test_cases:
        ...
        assert result.exit_code == 2, f"Command {args} should fail at option parsing"
...
def test_gc_005_multiple_unknown_options():
    ...
    assert result.exit_code == 2, "Multiple unknown options should cause exit code 2"

    # Should mention at least one unknown option
    output_lower = result.output.lower()
    assert (
        "no such option" in output_lower or "unrecognized" in output_lower
...
def test_gc_005_unknown_short_options():
    ...
    assert result.exit_code == 2, "Unknown short option should cause exit code 2"
...
def test_gc_005_typo_in_known_option():
    ...
    assert result.exit_code == 2, "Typo in option name should cause exit code 2"
