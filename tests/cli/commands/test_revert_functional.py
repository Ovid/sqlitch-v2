...
"""
    ...
    (project_dir / "deploy" / "users.sql").write_text("""-- Deploy revert_test:users to sqlite
...
""")
    ...
    (project_dir / "revert" / "users.sql").write_text("""-- Revert revert_test:users from sqlite
...
""")
    ...
    (project_dir / "verify" / "users.sql").write_text("""-- Verify revert_test:users on sqlite

SELECT id, username FROM users WHERE 0 = 1;
""")
    
    # Create deploy/revert/verify scripts for posts
    (project_dir / "deploy" / "posts.sql").write_text("""-- Deploy revert_test:posts to sqlite
...
""")
    ...
    (project_dir / "revert" / "posts.sql").write_text("""-- Revert revert_test:posts from sqlite
...
""")
    ...
    (project_dir / "verify" / "posts.sql").write_text("""-- Verify revert_test:posts on sqlite

SELECT id, user_id, title FROM posts WHERE 0 = 1;
""")
    
    # Create deploy/revert/verify scripts for comments
    (project_dir / "deploy" / "comments.sql").write_text("""-- Deploy revert_test:comments to sqlite
...
""")
    ...
    (project_dir / "revert" / "comments.sql").write_text("""-- Revert revert_test:comments from sqlite
...
""")
    ...
    (project_dir / "verify" / "comments.sql").write_text("""-- Verify revert_test:comments on sqlite
...
class TestRevertAllChanges:
    ...
    def test_revert_all_changes_in_reverse_order(
...
"""
        ...
        # Users scripts
        (project_dir / "deploy" / "users.sql").write_text("""-- Deploy tag_test:users to sqlite
BEGIN;
CREATE TABLE users (id INTEGER PRIMARY KEY);
COMMIT;
""")
        (project_dir / "revert" / "users.sql").write_text("""-- Revert tag_test:users from sqlite
BEGIN;
DROP TABLE users;
COMMIT;
""")
        (project_dir / "verify" / "users.sql").write_text("""-- Verify tag_test:users on sqlite
SELECT id FROM users WHERE 0 = 1;
""")
        
        # Posts scripts
        (project_dir / "deploy" / "posts.sql").write_text("""-- Deploy tag_test:posts to sqlite
BEGIN;
CREATE TABLE posts (id INTEGER PRIMARY KEY, user_id INTEGER REFERENCES users(id));
COMMIT;
""")
        (project_dir / "revert" / "posts.sql").write_text("""-- Revert tag_test:posts from sqlite
BEGIN;
DROP TABLE posts;
COMMIT;
""")
        (project_dir / "verify" / "posts.sql").write_text("""-- Verify tag_test:posts on sqlite
...
class TestRevertScriptExecution:
    ...
    def test_revert_removes_from_registry(
        self, project_with_deployed_changes: Path
    ) -> None:
...
"""
        ...
        (project_dir / "deploy" / "users.sql").write_text("""-- Deploy error_test:users to sqlite
BEGIN;
CREATE TABLE users (id INTEGER PRIMARY KEY);
COMMIT;
""")
        ...
        (project_dir / "revert" / "users.sql").write_text("""-- Revert error_test:users from sqlite
BEGIN;
THIS IS NOT VALID SQL;
COMMIT;
""")
        
        (project_dir / "verify" / "users.sql").write_text("""-- Verify error_test:users on sqlite
SELECT id FROM users WHERE 0 = 1;
""")
...
class TestRevertValidation:
    ...
    def test_revert_rejects_both_to_change_and_to_tag(
        ...
        assert result.exit_code != 0
        assert "cannot combine" in result.output.lower() or "mutually exclusive" in result.output.lower()
    
    def test_revert_validates_change_exists(
        self, project_with_deployed_changes: Path
    ) -> None:
...
class TestRevertConfirmation:
    ...
    def test_revert_requires_confirmation(
        self, project_with_deployed_changes: Path
    ) -> None:
