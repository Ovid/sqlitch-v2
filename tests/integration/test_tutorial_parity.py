...
def test_log_output_matches_sqitch(tmp_path: Path) -> None:
    ...
    with isolated_test_context(runner, base_dir=tmp_path) as (runner, temp_dir):
        ...
        assert log_lines[5] == ""
        assert log_lines[6] == "    Creates table to track our users."
        assert log_lines[-1] == ""
...
""",
        ...
        # Use relative path for both registry creation and revert command
        workspace_uri_relative = "db:sqlite:flipr_test.db"
        registry_uri = derive_sqlite_registry_uri(
            workspace_uri=workspace_uri_relative,
            project_root=Path.cwd(),
        )
        registry_path = resolve_sqlite_filesystem_path(registry_uri)
        registry_path.parent.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            # Insert dependency record for flips -> users
            cursor.execute(
                """
        ...
        result = runner.invoke(main, ["revert", workspace_uri_relative, "-y"])
