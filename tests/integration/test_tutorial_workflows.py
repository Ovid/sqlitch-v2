        ...
        with isolated_test_context(runner, base_dir=tmp_path) as (runner, temp_dir):
            ...
            result = runner.invoke(
                ...
                [
                    ...
                    "flipr",
                    "--uri",
                    "https://github.com/example/flipr/",
                    "--engine",
                    "sqlite",
                ],
            )
...
"""
            ...
            runner.invoke(
                main, ["add", "flips", "--requires", "users", "-n", "Creates flips table."]
            )
            Path("deploy/flips.sql").write_text(
                """-- Deploy flipr:flips to sqlite
BEGIN;
...
"""
            ...
            Path("verify/userflips.sql").write_text(
                "SELECT username, message FROM userflips WHERE 0;"
            )
            ...
            with closing(sqlite3.connect("sqitch.db")) as registry:
                deployed = [
                    ...
                    for row in registry.execute(
                        "SELECT change FROM changes ORDER BY committed_at"
                    ).fetchall()
                ]
