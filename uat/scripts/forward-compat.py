#!/usr/bin/env python3
"""sqlitch → sqitch forward compatibility harness placeholder.

This script will eventually replay the SQLite tutorial using SQLitch first and
then validate Sqitch can continue the workflow. For lockdown we provide a
sanity-check CLI shell so failing tests can drive the implementation in T116.
"""
...
def main() -> int:
    ...
    placeholder = (
        "Forward compatibility execution is not yet implemented. "
        "Set SQLITCH_UAT_SKIP_EXECUTION=1 during development or implement T116."
    )
    print(placeholder)
    sanitized = sanitization.sanitize_output(placeholder)
    _write_log(log_path, [header, sanitized])

    # For now exit with non-zero to signal missing implementation when skip is disabled.
    return 1
