#!/usr/bin/env python3
"""sqlitch → sqitch forward compatibility harness.

This script replays the SQLite tutorial using SQLitch first for each step,
then validates that Sqitch can continue the workflow from that state. This
tests forward compatibility: can Sqitch work with databases deployed by SQLitch?

The workflow alternates:
  1. SQLitch executes step N
  2. Sqitch executes step N+1
  3. SQLitch executes step N+2
  4. And so on...

This ensures that Sqitch can always pick up where SQLitch left off.
"""
...
WORK_DIR = Path("uat/forward_compat_results")
LOG_FILE = WORK_DIR / "forward-compat.log"
...
def main() -> int:
    ...
    header = "Forward compatibility harness (sqlitch → sqitch)"
    ...
    # Execute steps alternating between sqlitch and sqitch
    had_failure = False
    for i, step in enumerate(TUTORIAL_STEPS):
        # Alternate: even indices (0, 2, 4...) use sqlitch, odd use sqitch
        use_sqitch = (i % 2) == 1
    ...
    if had_failure:
        color_print(RED, "\n" + "=" * 60)
        color_print(RED, "  ❌ FORWARD COMPATIBILITY TEST FAILED")
        color_print(RED, "=" * 60)
        ...
        color_print(GREEN, "  Sqitch can continue workflows started by SQLitch")
        color_print(GREEN, "=" * 60)
