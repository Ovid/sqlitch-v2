...
def generate_report(audit_results: Dict[str, Dict], output_path: Path) -> None:
    ...
    commands_with_exits = sum(
        1 for r in audit_results.values() if r.get("has_explicit_exits", False)
    )
    ...
    commands_needing_review = [
        cmd for cmd, result in audit_results.items() if result.get("has_explicit_exits", False)
    ]
